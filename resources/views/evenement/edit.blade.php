<x-app-layout>
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ route('evenement.update', ['evenement' => $evenement->id]) }}" class="max-w-md mx-auto">
        @csrf
        @method('PUT')

        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="title" id="title" value="{{ old('title', $evenement->title) }}"
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="date" name="date" id="date" value="{{ old('date', $evenement->date) }}"
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" " required />
            <label for="date"
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="location" id="location" value="{{ old('location', $evenement->location) }}"
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" " required />
            <label for="location"
        ...
        <div class="relative z-0 w-full mb-5 group">
            <select name="categorie_id" id="categorie_id"
                ...
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}"  {{ old('categorie_id', $evenement->categorie_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                @endforeach
            </select>
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" name="available_places" id="available_places" value="{{ old('available_places', $evenement->available_places) }}"
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" " required />
            <label for="available_places"
                class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Available
                Places</label>
        </div>


        <div class="relative z-0 w-full mb-5 group">
            <select name="type_of_reservation" id="type_of_reservation"
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                required>
                <option value="Automatique"  {{ old('type_of_reservation', $evenement->type_of_reservation) == 'Automatique' ? 'selected' : '' }} >Automatic</option>
                <option value="par_confirmation" {{ old('type_of_reservation', $evenement->type_of_reservation) == 'par_confirmation' ? 'selected' : '' }}  >By confirmation</option>
            </select>
            <label for="type_of_reservation"
        ...
        {{-- <div class="relative z-0 w-full mb-5 group">
            <select name="type_of_reservation" id="type_of_reservation" required>
                <option value="Automatique" {{ old('type_of_reservation', $evenement->type_of_reservation) == 'Automatique' ? 'selected' : '' }}>Automatic</option>
                <option value="par_confirmation" {{ old('type_of_reservation', $evenement->type_of_reservation) == 'par_confirmation' ? 'selected' : '' }}>By confirmation</option>
            </select>
            <label for="type_of_reservation"
